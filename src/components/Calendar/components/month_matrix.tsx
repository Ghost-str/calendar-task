...
import useDayNamesList, { DayNamesList as DayNamesListType } from "../hooks/use_day_names_list";
...
export default function MonthMatrix({dayNames, currentMonth, onChange}: MonthMatrixProps) {
    return <table>
...
function WeekList({currentMonth, onChange}: WeekListProps) {
    const currentMonthMatrix = useMemo(()=> monthMatrix(currentMonth), [currentMonth]);
              ...
              return <tr key={index}>
                {
                  week.map((day, index)=> {
                    return <td key={index}>
                        <DayItem day={day} currentMonth={currentMonth} onChange={onChange}/>
                    </td>
                  })
                }
              </tr>
